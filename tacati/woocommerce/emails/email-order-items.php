...
foreach ($items as $item) :
	...
	<tr>
		<td style="text-align:left; vertical-align:middle; border: 1px solid #eee;"><?php
			...
			// File URL
			echo 	( $show_download_links && $_product->exists() && $_product->is_downloadable() && $_product->has_file() ) ? '<br/><small>' . __( 'Download:', TACATI_TD ) . ' <a href="' . $order->get_downloadable_file_url( $item['id'], $item['variation_id'] ) . '" target="_blank">' . $order->get_downloadable_file_url( $item['id'], $item['variation_id'] ) . '</a></small>' : '';
		...
		<td style="text-align:left; vertical-align:middle; border: 1px solid #eee;"><?php echo get_formatted_line_subtotal_custom( $order, $item ); ?></td>
	</tr>
