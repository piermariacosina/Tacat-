...
if( ! $home ) 
	$home = _x( 'Home', 'breadcrumb', TACATI_TD );
...
if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == woocommerce_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( is_category() ) {
		...
		$queried_object = $wp_query->get_queried_object();
		echo $prepend . $before . __('Products tagged &ldquo;', TACATI_TD) . $queried_object->name . '&rdquo;' . $after;
		...
		if ( is_search() ) {
			...
			echo $before . '<a href="' . get_post_type_archive_link('product') . '">' . $_name . '</a>' . $delimiter . __('Search results for &ldquo;', TACATI_TD) . get_search_query() . '&rdquo;' . $after;
		...
		echo $before . __( 'Error 404', TACATI_TD ) . $after;
		...
		echo $before . __( 'Search results for &ldquo;', TACATI_TD ) . get_search_query() . '&rdquo;' . $after;

	} elseif ( is_tag() ) {

			echo $before . __( 'Posts tagged &ldquo;', TACATI_TD ) . single_tag_title('', false) . '&rdquo;' . $after;

	} elseif ( is_author() ) {

		$userdata = get_userdata($author);
		echo $before . __( 'Author:', TACATI_TD ) . ' ' . $userdata->display_name . $after;

	}

	if ( get_query_var( 'paged' ) )
		echo ' (' . __( 'Page', TACATI_TD ) . ' ' . get_query_var( 'paged' ) . ')';
