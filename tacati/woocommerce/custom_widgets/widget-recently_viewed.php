...
class WooCommerce_Widget_Recently_Viewed_CUSTOM extends WP_Widget {
	...
	function WooCommerce_Widget_Recently_Viewed_CUSTOM() {
		...
		$this->woo_widget_description = __( 'Display a list of recently viewed products.', TACATI_TD );
		$this->woo_widget_idbase = 'woocommerce_recently_viewed_products';
		$this->woo_widget_name = __('Custom WooCommerce Recently Viewed Products', TACATI_TD );

		/* Widget settings. */
		$widget_ops = array( 'classname' => $this->woo_widget_cssclass, 'description' => $this->woo_widget_description );

		/* Create the widget. */
		$this->WP_Widget('custom_recently_viewed_products', $this->woo_widget_name, $widget_ops);
	...
	function widget($args, $instance) {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Recently viewed', TACATI_TD) : $instance['title'], $instance, $this->id_base);
		...
		echo '</h1>';
		echo '<div class="icon visti_di_recente"></div>';?>
		<ul class="product_list_widget">
		<?php  while ($r->have_posts()) : $r->the_post(); global $product; ?>
		<li>
			<h5><a href="<?php the_permalink() ?>" title="<?php echo esc_attr(get_the_title() ? get_the_title() : get_the_ID()); ?>">
	<?php if (has_post_thumbnail()) the_post_thumbnail('shop_thumbnail'); else echo '<img src="'. woocommerce_placeholder_img_src() .'" alt="Placeholder" width="'.$woocommerce->get_image_size('shop_thumbnail_image_width').'" height="'.$woocommerce->get_image_size('shop_thumbnail_image_height').'" />'; ?><?php if ( get_the_title() ) the_title(); else the_ID(); ?></a></h5>
		...
		<div class="price">
				...
				<p>1 ×<span><?php echo $product->get_price_html(); ?></span></p>    
			</div>
			<hr>	
...
?>
		...
		<p><label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of products to show:', TACATI_TD); ?></label>
